...
def count_fingers(hand_landmarks, hand_label):
    ...
    # Keep return signature same as your code
    count = 0
    finger_status = []
...
def inside_bb(x, y, bb):
    return (x > bb[0]) and (x < bb[2]) and (y > bb[1]) and (y < bb[3])


def draw_virtual_joystick_overlay(frame):
    # Uses current global colors (color_left/up_left/... etc.)
    cv2.rectangle(frame, (left_arm_bb[0], left_arm_bb[1]),
                  (left_arm_bb[2], left_arm_bb[3]),
                  color_left, 2)
    cv2.putText(frame, "UP", (140, 80), cv2.FONT_HERSHEY_PLAIN, 2, up_left, 3)
    cv2.putText(frame, "DOWN", (120, 260), cv2.FONT_HERSHEY_PLAIN, 2, down_left, 3)
    cv2.putText(frame, "<-", (53, 165), cv2.FONT_HERSHEY_PLAIN, 2, left_left, 3)
    cv2.putText(frame, "->", (220, 165), cv2.FONT_HERSHEY_PLAIN, 2, right_left, 3)

    cv2.rectangle(frame, (right_arm_bb[0], right_arm_bb[1]),
                  (right_arm_bb[2], right_arm_bb[3]),
                  color_right, 2)
    cv2.putText(frame, "UP", (440, 80), cv2.FONT_HERSHEY_PLAIN, 2, up_right, 3)
    cv2.putText(frame, "DOWN", (420, 260), cv2.FONT_HERSHEY_PLAIN, 2, down_right, 3)
    cv2.putText(frame, "<-", (353, 165), cv2.FONT_HERSHEY_PLAIN, 2, left_right, 3)
    cv2.putText(frame, "->", (520, 165), cv2.FONT_HERSHEY_PLAIN, 2, right_right, 3)
...
with mp_hands.Hands(min_detection_confidence=0.5,
    ...
    while cap.isOpened() and (not rospy.is_shutdown()):
        ...
        # Reset finger states each frame
        left_hand = [0, 0, 0, 0, 0]
        right_hand = [0, 0, 0, 0, 0]
        ...
        # For joystick: store fingertip coords per hand (index finger tip = landmark 8)
        rx = ry = None
        lx = ly = None

        # Draw mode text always
        cv2.putText(frame, mode, (20, 50), cv2.FONT_HERSHEY_PLAIN, 2, (41, 36, 14), 5)

        # If hands are detected, update finger states + store fingertip coords + draw landmarks
        if results.multi_hand_landmarks:
            for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                # For wave logic (landmark 9 x)
                wave_x = int(hand_landmarks.landmark[9].x * w)
        ...
        # ---------------------------------------------------------
        # MODE 0: Virtual Joystick (overlay ALWAYS visible)
        # ---------------------------------------------------------
        if choice == 0:
            # Default overlay colors every frame
            color_left = green
            up_left = green
            down_left = green
            left_left = green
            ...
            # Decide which fingertip to use (prefer a hand with index finger up)
            candidates = []

            if right_hand[1] == 1 and rx is not None and ry is not None:
                candidates.append(("Right", rx, ry))
            if left_hand[1] == 1 and lx is not None and ly is not None:
                candidates.append(("Left", lx, ly))

            # If we have a pointing finger, use the first one that lies inside either bb
            chosen = None
            for label, x1, y1 in candidates:
                if inside_bb(x1, y1, left_arm_bb) or inside_bb(x1, y1, right_arm_bb):
                    chosen = (label, x1, y1)
                    break
            # If none are inside, still allow choosing first candidate (optional). Keep old behavior: only act inside bb.
            if chosen is not None:
                _, x1, y1 = chosen

                # Left virtual joystick area
                if inside_bb(x1, y1, left_arm_bb):
                    color_left = purple
                ...
                # Right virtual joystick area
                elif inside_bb(x1, y1, right_arm_bb):
                    color_right = purple
                    cv2.circle(frame, (x1, y1), 15, (255, 0, 255), cv2.FILLED)
            ...
            # Draw overlay ALWAYS (even if no hand/finger detected)
            draw_virtual_joystick_overlay(frame)

        # ---------------------------------------------------------
        # MODE 1: Gesture recognition (your original logic)
        # ---------------------------------------------------------
        elif choice == 1:
            if results.multi_hand_landmarks:
                if right_hand == [0, 0, 0, 0, 0] and left_hand == [0, 0, 0, 0, 0]:
                    ...
                    if wave_val[0] == 0 and wave_val[1] == 0:
                        wave_val[0] = wave_x
                        wave_val[1] = wave_x
                    else:
                        wave_val[0] = min(wave_x, wave_val[0])
                        wave_val[1] = max(wave_x, wave_val[1])
                    dist = abs(wave_val[0] - wave_val[1])

                    if (dist > 640 * 0.4):
                        print("Distance Covered: ", "{:.2f}".format((dist / 640) * 100), "%")
                        wave = 31
        ...
        # Press 'q' to exit the loop (if you enable cv2.imshow)
